...
| Selected-artwork name, X, Y, width, height, rotation, and proportions controls remain explicitly named after compacting layer rows. | `app::ui_tests::layer_transform_fields_have_associated_accessible_names` |
| Icon-only alignment and layer controls expose verb-object names, selected state, and targets of at least 32×32 points. | `app::ui_tests::sidebar_icon_controls_have_clear_names_states_and_targets` |
